    ...
    def infer(self, instruction, scene_description):
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                self.get_logger().info(f"   API原始返回: {text}")
                import re
...
class SceneDescriptionGenerator:
    ...
    def generate(self, pos):
        ...
        if dist < 2.0:
            return "A green trash bin on the roadside, clearly visible. It is plastic, green, and placed next to a tree."
        elif dist < 5.0:
            return "A roadside area with a green trash bin visible in the distance. The bin is about 5 meters away."
        elif dist < 10.0:
            return "A grassy area with a path and some trees. You can see a green trash bin in the distance."
        else:
            return "An open field with scattered trees. No trash bin visible."
...
class AirHuntNode(Node):
    ...
    def vlm_loop(self):
        while rclpy.ok() and not self.found:
            ...
            self.get_logger().info(f"   描述: {scene[:60]}...")
            ...
            self.get_logger().info(f"🎯 目标: ({target[0]:.1f}, {target[1]:.1f}), 价值={best_val:.3f}")

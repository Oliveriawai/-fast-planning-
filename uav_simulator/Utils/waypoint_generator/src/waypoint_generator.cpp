...
void publish_waypoints() {
    ...
    // 添加日志打印要发布的路径点数量和具体坐标
    std::stringstream ss;
    ss << "Publishing /waypoint_generator/waypoints with " << waypoints.poses.size() << " points: ";
    for (size_t i = 0; i < waypoints.poses.size(); ++i) {
        const auto& pose = waypoints.poses[i].pose.position;
        ss << "[x: " << pose.x << ", y: " << pose.y << ", z: " << pose.z << "]";
        if (i < waypoints.poses.size() - 1) ss << ", ";
    }
...
void goal_callback(const geometry_msgs::msg::PoseStamped::SharedPtr msg) {
    ...
    RCLCPP_INFO(this->get_logger(), "Received /move_base_simple/goal: [x: %.2f, y: %.2f, z: %.2f]", msg->pose.position.x, msg->pose.position.y, msg->pose.position.z);
